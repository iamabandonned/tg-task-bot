...
const API = {
  ...
  async deleteTasks(ids) {
    ...
    setState('tasks', tasks.filter(t => !ids.includes(t.id)));
    return { success: true };
  },
  ...
  async deleteProject(id) {
    ...
    setState('projects', projects.filter(p => !idsToDelete.includes(p.id)));
    return { success: true };
  },
  ...
  async deleteProjects(ids) {
    ...
    setState('projects', projects.filter(p => !allIds.includes(p.id)));
    return { success: true };
  },
  ...
  async deleteEmployees(ids) {
    await this.delay();
    const employees = getState('employees') || [];
    setState('employees', employees.filter(e => !ids.includes(e.id)));
    return { success: true };
  },
  ...
  async getAnalytics(filters = {}) {
    ...
    const tasksByEmployee = employees.map(emp => {
      ...
      return {
        ...
        overdue: empTasks.filter(t => isOverdue(t.scheduledDate) && t.status !== 'completed').length
      };
